...
class ajaxSpider(object):
    ...
    def scan(self, url='', inscope='', contextname='', subtreeonly='', apikey=''):
        ...
        This component is optional and therefore the API will only work if it is installed
        """
        return next(iter(self.zap._request(self.zap.base + 'ajaxSpider/action/scan/', {'url' : url, 'inScope' : inscope, 'contextName' : contextname, 'subtreeOnly' : subtreeonly, 'apikey' : apikey}).values()))

    def scan_as_user(self, contextname, username, url='', subtreeonly='', apikey=''):
        """
        This component is optional and therefore the API will only work if it is installed
        """
        return next(iter(self.zap._request(self.zap.base + 'ajaxSpider/action/scanAsUser/', {'contextName' : contextname, 'userName' : username, 'url' : url, 'subtreeOnly' : subtreeonly, 'apikey' : apikey}).values()))
